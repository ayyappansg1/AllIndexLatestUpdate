...
public class CreatePortFolio extends BaseClass {
	...
	@Test(priority = 11)
	public void verifyPreviousDataDeleteIfNewUploaded() throws InterruptedException, AWTException {
		logger.info("Running method verifyPreviousDataDeleteIfNewUploaded ");
		manualSearchPage=new ManualSearchpage(driver);
		createPortFolioManual.clickCreatePortfolio();
		manualSearchPage.clickManualSearch();
		manualSearchPage.sendTextToTheSearchBox(Constants.stock1,Constants.stock2,Constants.stock3);
		manualSearchPage.uploadExcelFileWithAllField(driver);
		manualSearchPage.clickNextButtonAfterUpload();
		List<String> allText = manualSearchPage.getAllText();
		List<String> expectedList=new LinkedList<String>();
		expectedList.add("Cerner Corporation");
		expectedList.add("Apple Inc");
		expectedList.add("Microsoft Corporation");
		Assert.assertEquals(allText.size(), 3,"Verifying the count of Selected Stocks After uploading from excel file");
		assertThat(allText).hasSameElementsAs(expectedList);
	}
	@Test(priority = 12)
	public void verifyWeightsEquallyDistributedEvenMissedWeightUsedInExcel() throws InterruptedException, AWTException {
		logger.info("Running method checkWarningMEssageIfExcelUploadedWithMissedWeighing ");
		manualSearchPage=new ManualSearchpage(driver);
		createPortFolioManual.clickCreatePortfolio();
		manualSearchPage.clickManualSearch();
	...
	@Test(priority = 13)
	public void verifyWeightsRemainsAsEvenStocksFromExcelAndManuallySearchh() throws InterruptedException, AWTException {
		logger.info("Running method checkWarningMEssageIfExcelUploadedWithMissedWeighing ");
		manualSearchPage=new ManualSearchpage(driver);
		createPortFolioManual.clickCreatePortfolio();
		manualSearchPage.clickManualSearch();
		...
		List<String> equallyDividedWeight = manualSearchPage.getEquallyDividedWeight();
		Assert.assertEquals(equallyDividedWeight.get(0), "40");
		Assert.assertEquals(equallyDividedWeight.get(1), "20");
		Assert.assertEquals(equallyDividedWeight.get(2), "40");
	}
	@Test(priority = 14)
	public void inCaseOfPerStockWeighingDeSelectsAdditionalScreen() throws InterruptedException, AWTException {
		logger.info("Running method checkWarningMEssageIfExcelUploadedWithMissedWeighing ");
		manualSearchPage=new ManualSearchpage(driver);
		createPortFolioManual.clickCreatePortfolio();
		manualSearchPage.clickManualSearch();
		...
		manualSearchPage.clickBackButton();
		manualSearchPage.deselectOneSelectStocks();
		manualSearchPage.clickNextbutton();

		List<String> equallyDividedWeightNew = manualSearchPage.getEquallyDividedWeight();
		assertThat(equallyDividedWeight).hasSameElementsAs(equallyDividedWeightNew);
	}
	@Test(priority = 15)
	public void checkManuallySelectedStockResetIfExcelFileUploadedInLaterSteps() throws InterruptedException, AWTException {
		logger.info("Running method checkWarningMEssageIfExcelUploadedWithMissedWeighing ");
		manualSearchPage=new ManualSearchpage(driver);
		createPortFolioManual.clickCreatePortfolio();
		manualSearchPage.clickManualSearch();
		Properties prop = new TestUtil().init_Properties();
		manualSearchPage.sendTextToTheSearchBox(prop.getProperty("stockCheck1"),prop.getProperty("stockCheck2"),prop.getProperty("stockCheck3"));
		manualSearchPage.clickNextbutton();
		manualSearchPage.clickResetToEqualWeight();
		List<String> equallyDividedWeightLabels = manualSearchPage.getEquallyDividedWeightLabels();

		manualSearchPage.uploadExcelFileWithTwoFieldsOnly(driver);
		manualSearchPage.clickNextButtonAfterUpload();
		manualSearchPage.clickNextbutton();
		manualSearchPage.clickResetToMarketCap();

		//manualSearchPage.clickResetToEqualWeight();
		List<String> equallyDividedWeightLabelsNew = manualSearchPage.getEquallyDividedWeightLabels();
		Assert.assertEquals(equallyDividedWeightLabelsNew.size(),equallyDividedWeightLabels.size(),"Checking size of both list");
	}
}
